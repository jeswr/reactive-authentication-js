...
const providerFor = ({ getItem, setItem, removeItem }) =>
    new DPoPTokenProvider(callback, unreachable, unreachable, unreachable, { getItem, setItem, removeItem })
...
test("restored IndexedDB credentials produce a fresh request-bound proof each time", async () => {
    await createIndexedDbStore({ namespace: "tokens" }).setItem(url, await entry())
    const provider = providerFor(createIndexedDbStore({ namespace: "tokens" }))
...
test("two providers sharing persistent credentials rotate once and await durable writes", async t => {
    const cache = createIndexedDbStore({ namespace: "rotation" })
    const original = await entry(true)
    await cache.setItem(url, original)
    let grants = 0
    t.mock.method(globalThis, "fetch", async (input, options) => {
        ...
        assert.equal(await cache.getItem(url), null, "consumed token must not remain durable")
        grants++
    ...
    const results = await Promise.all([
        ...
        providerFor(createIndexedDbStore({ namespace: "rotation" })).upgrade(new Request(url)),
    ])
    assert.equal(grants, 1)
    assert.ok(results.every(result => result.headers.get("Authorization") === "DPoP renewed"))
    const restored = await createIndexedDbStore({ namespace: "rotation" }).getItem(url)
...
test("refresh without rotation retains the previous refresh token", async t => {
    ...
    await cache.setItem(url, await entry(true))
    ...
    assert.equal((await cache.getItem(url)).tokenResult.refresh_token, "refresh-1")
})
...
test("failed durable rotation does not expose credentials or leave the consumed refresh token", async t => {
    ...
    t.mock.method(cache, "setItem", async () => { throw new Error("quota") })
    await assert.rejects(providerFor(cache).upgrade(new Request(url)), /quota/)
    assert.equal(await cache.getItem(url), null)
})

test("failed invalidation prevents consumption of the refresh token", async t => {
    const cache = createMemoryStore()
    await cache.setItem(url, await entry(true))
    t.mock.method(cache, "removeItem", async () => { throw new Error("denied") })
...
test("fresh authorization caches only successful results and preserves constructor defaults", async t => {
    ...
    assert.equal((await cache.getItem(url)).tokenResult.access_token, "fresh")
